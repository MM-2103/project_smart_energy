...
## .NET Framework
Deze applicatie is gerealiseerd in .NET 9. Je kunt controleren of je de SDK (Software Development Kit) voor .NET 9 geinstalleerd hebt via het volgende commando in je command line interface:

`dotnet --info`

## Configuratie van je SmartMeter
In de `Program.cs` vind je de `SmartMeterConfiguration` die wordt gebruikt bij het opstarten van de webapplicatie. Vervang hier de waarde van `MeterId` met de _decimale_ waarde van de HEX-code van je SmartMeter.
